...
export default function Create(props) {
    ...
    const yearValue = yearOptions.find(obj => {
        return obj.value === data.expiry_year;
    });
    ...
    function changeTotal(plan_id, expiry_year, customclients, couponcode) {
        ...
        if (plan_id) {
            ...
            if (plan_id == 2) {
                var tempsubtotal = Number(plan.price*expiry_year*customclients).toFixed(2);
                $('#coupon-input-text-box').show();
            }else{
                var tempsubtotal = Number(plan.price*expiry_year).toFixed(2);
                $('#coupon-input-text-box').hide();
            }
            ...
            if(customclients == true){
                if (plan_id == 1) {
                    // setPricing('$'+Number(tempsubtotal).toFixed(2)+'/14 days + Tax');
                    ...
                    if (plan_id == 2) {
                        ...
                        setPricing('$'+Number(tempsubtotal).toFixed(2)+' ('+plan.qty+' Server +'+plan.qty+' Client x '+expiry_year+'-Year)');
                        setServerqty(plan.qty);

                    }
    
                    // setPricing('$'+Number(tempsubtotal).toFixed(2)+'/'+expiry_year+' year + Tax');
                }
            }else{
                setServerqty(0)
                 if (plan_id == 1) {
                    ...
                    if (plan_id == 2) {
                        ...
                        setPricing('$'+Number(tempsubtotal).toFixed(2)+' (1 Server + '+plan.qty+' Client x '+expiry_year+'-Year)');
            ...
            /* var temprate = Number(plan?.coupon?.rate);
            var tempcouponprice = 0;
            if (temprate > 0) {
                tempcouponprice = ((parseFloat(tempsubtotal)*parseFloat(temprate))/100).toFixed(2);
                tempsubtotal = tempsubtotal-tempcouponprice;
            ...
            if(customclients == true){
                var standalonetotal = ((parseFloat(tempsubtotal)*standaloneval)/100).toFixed(2);
    ...
    /* function couponCheckCode(couponcode){
        // var couponcode = $("#coupon-input-box").val();
    ...
    return (
        ...
        >
            ...
            <form name="createForm" onSubmit={handleSubmit}>
                ...
                {/* <Form data={data} errors={errors} setData={setData} types={props.types} products={props.products} /> */}
                            ...
                            <table className="table">
                                <thead className="table-light">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2" style={{display: standaloneval > 0 ? "block":"none"}}>Standalone Mode</th>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="border px-4 py-2 stanlon" style={{display: standaloneval > 0 ? "block":"none"}}>
                    ...
                    <div className='col-md-4'>
                        ...
                        <div className="table-responsive">
                            <table className="table">
                                <tbody>
                                    {/* <tr>
                                        <td className="border px-4 py-2 text-right"><span className='checkoutTotalText'>${subtotal}</span></td>
                                    </tr> */}
                                    <tr>
                                        <td className="border px-4 py-2 text-right"><span className='checkoutTotalTextTitle'>Subtotal :</span> <span className='checkoutTotalText'>
                                            {subtotal == '-'?'-':'$'+subtotal}
                                        </span></td>
                                    </tr>
                                 
                                    <tr >
                                        <td className="border px-4 py-2 text-right" style={{display: standalonetot != 0 ? "block":"none"}}><span className='checkoutTotalTaxTextTitle'>
                                        Standalone Surcharge : {standalonetot == '-'?'-':+standaloneval+'% (+$'+standalonetot+')'}
                                        </span></td>
                                    </tr>
                                    <tr>
                                        <td className="border px-4 py-2 text-right">
                                            <span className='checkoutTotalTaxTextTitle'>
                                            Discount : {coupon == '-'?'-':coupon+'% (-$'+couponprice+') OFF'}
                                            </span>
                                     ...
                                     <tr>
                                        <td className="border px-4 py-2 text-right"><span className='checkoutTotalTaxTextTitle'>
                                            Sales Tax : {tax == '-'?'-':'10.5% (+$'+tax+')'}
                                        </span></td>
                                    </tr>
                                    {/* <tr>
                                        <td className="border px-4 py-2 text-right">Coupon Code</td>
                                    </tr>
                                    <tr>
                                        <td className="border px-4 py-2 text-right"><span className='checkoutTotalTaxTextTitle'>{coupon}% OFF</span></td>
                                    </tr> */}
                                    ...
                                    <tr>
                                        <td className="border px-4 py-2 text-right">
                                            <span className='checkoutTotalTextTitle'>Total :</span> <span className='checkoutTotalText'>{total == '-'?'-':'$'+total}</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="border px-4 py-2">
                                            <div className="mt-4 formsubmitbutton">
                                                ...
                                                <Link
                                                    ...
                                                    style={{marginLeft: "0px",width: "100%",textAlign: "center"}}
                                                >
                                                    Back
                                                </Link>
